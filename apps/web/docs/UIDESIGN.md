...
统一容器规格：所有 Frame 为 `Route Surface` 的页面（工作台、创建研究、研究任务、问答历史、知识库列表与详情等）共享同一基底——最大宽度 1260px、侧边内边距 40px（内容宽约 1180px）、`--es-surface` + 1px `--es-border-default` + `--es-radius-major`，不使用阴影。1280 × 720 基准视口下，容器外保留 `10px 10px 0` 的浅灰 Canvas 边距，容器高度为 `calc(100vh - 46px)`；各 Route Surface 页面不得维护不同的外框尺寸。
...
- `Luminous Primary`：高价值创建/进入动作。浅色主题下为深色实底白字；深色主题下为月白底带克制白色渐变光；文案后带 `↗` 箭头（对齐原型 `.luminous-button`）；
...
按钮配色统一为中性黑/白：主按钮/CTA 近黑实底白字（浅色）或近白实底黑字（深色）；Ghost/Text 次级文字按钮与低风险跳转链接用中性 ink（`--es-ink-primary`/`--es-ink-secondary`）。**任何按钮不得使用品牌蓝（`--es-moonstone`/`--es-knowledge`）作为填充或文字色**；moonstone 只保留给选中态、徽章、进度、hover 强调与焦点环。
...
- 主列顶部“快速开始”是 `Inset Panel`，内部最多两个白色 `Action Card`：快速提问、深度研究。每卡包含图标、标题、说明和明确动作，整卡或按钮命中行为必须一致；卡内标题/说明文案居左，动作文案（进入问答、定义范围）位于卡片右下侧、与标题文案同一水平线。
- `RECENT RESEARCH` 是连续 Ledger：标题、状态、更新时间和动作列对齐；无数据时在同一列表区域显示真实空态，不制造统计数字。空态中动作主按钮位于区块右侧、"查看全部"下方，与空态说明句处于同一水平线（说明居左、动作居右）。
- 空态动作布局：`EmptyState` 支持 `actionAlign="below"`（动作位于说明文案下方，默认）与 `actionAlign="beside"`（动作与说明文案同一行、右对齐）两种形态。
...
- 搜索和 `all|mine|public` 范围位于独立工具栏，工具栏上下各用一条 Hairline；搜索框不附加独立提交按钮。
- 知识库按 Ledger 行呈现，固定列顺序为“知识库/描述｜可见性｜文档｜索引状态｜最近更新｜操作”；操作表头与下方操作按钮组居中对齐（`justify-self:center` + 按钮组 `justify-content:center`），其余表头与数据左缘对齐，禁止一库一卡。
- 操作列只保留主操作“进入知识库”，编辑/删除收敛到 `⋮` 溢出菜单（Owner 或管理员治理可见，只读成员不显示），与问答历史行菜单同一视觉语言。
- Ledger 表头固定、中间行区独立滚动、分页固定在 Route Surface 底部；单行或少量行保持内容行高，不拉伸填满剩余高度。
- 时间使用 `YYYY-MM-DD HH:mm` 绝对格式；公开与私有徽章使用不同色调，索引状态同时显示文字。
- 加载、真正无数据、筛选无匹配和错误态占用同一 Ledger 区域；空态与错误态内容居中，恢复动作使用统一 Button。
- 页尾可显示范围/权限说明，但不得用不可验证的全局文档总数作为宣传指标。

### 7.10 知识库详情 `/knowledge-bases/:kbId`

- 使用 Route Surface；顶部 Hero 是共享行轨道的左右两列：左列为 eyebrow、名称和描述，右列为动作和元数据。名称与动作同行，描述首行与元数据同行；名称单行省略、描述最多三行且正文宽度不超过约 520px。Hero 动作只保留“用它提问”主按钮与 Owner 的上传文档；知识库编辑/删除不在详情页，由列表行 `⋮` 菜单承载。
- 元数据顺序固定为“Owner 创建｜可见性｜更新时间”；时间使用 `YYYY-MM-DD HH:mm`。公开徽章使用 knowledge 色、私有徽章使用中性色，普通元数据使用 muted 文字色。
- “Knowledge Pulse”是一个三等分 Metric Strip，只承载文档总数、分块总数和创建时间；三格值使用同一 Display 字体与 26px 字号，标签、数值和说明跨格对齐，不拆成 KPI 卡片。
- 文档区按“标题/工具栏｜固定表头｜可滚动行区｜分页/说明”纵向排列；列顺序为“文档｜状态｜有效片段｜更新时间｜操作”，操作表头与操作按钮组居中对齐。
- 六种文档状态使用独立语义色：queued/deleting 中性、processing cyan、completed success、partial warning、failed danger，并始终显示状态文字。
- 上传使用右侧 Drawer：常驻 Drop Zone 最小高度 200px，已选队列追加在其下并可继续添加；逐文件状态在队列内连续排列，Drawer 页脚动作居中。
- 创建/编辑 Drawer 的长度提示和字数计数与字段对齐；可见性选项的标题和说明采用两行结构。
- 切片原文使用较宽 Drawer；先显示来源身份与元数据，再显示实时鉴权后的正文或受限态。后台处理中提示位于 Ledger 下方的说明区，不漂浮为告警卡。
...
| 命令面板 | 全局（顶栏命令入口 / `⌘K`） | 顶栏按钮**原位展开为搜索输入框**（combobox，非居中浮层）：输入框尺寸与按钮一致（220×32px），输入有匹配才在其下方弹出 Ledger 命令列表；Escape/外部点击关闭并返回命令入口按钮（§7.22） |

Overlay 不是独立页面卡片。任何打开态都必须纳入对应页面的 light/dark 视觉验收；不得只验关闭态截图。

### 7.22 命令面板

命令入口由顶栏命令入口按钮或 `⌘K` 触发，承载页面导航与明确创建动作（FRONTEND §3.4）。

- 触发后按钮**原位展开为搜索输入框**（combobox，不弹出独立面板）：输入框尺寸与命令入口按钮一致（`min-width: 220px`、高 32px），聚焦态使用 `--es-focus` 边框 + `--es-radius-control`；输入框上方/内部是命令入口原位，`Escape` 或点击输入框外部关闭并恢复按钮。
- 输入框空时不显示命令列表；输入有匹配后才在输入框下方弹出列表。
- 命令列表锚定在输入框下方（`position: absolute; top: calc(100% + 8px); left: 0; right: 0`，宽度与输入框一致），使用 `--es-surface`、1px `--es-border-default`、`--es-radius-major` 与 `--es-shadow-popover`；不占满屏、不加深色遮罩。
- 命令列表为 Ledger：每行展示命令名（`--es-ink-primary`）与辅助说明（`--es-ink-muted`）；当前项与 Hover 使用 `--es-surface-muted` 高亮，当前项同时有左侧标记。
- 输入有值但无匹配时展示「无匹配命令」单行空态，不展示空 KPI。
- 命令执行后关闭列表并导航；关闭不改变页面滚动位置。深浅主题只替换 Token，结构与边界不变。
