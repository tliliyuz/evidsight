...
统一容器实现：所有 Frame 为 `Route Surface` 的页面（工作台、创建研究、研究任务、问答历史、知识库列表与详情等）共享 `.route-surface` 容器类——`max-width: 1260px`（对齐原型 `route-view`）、侧边 padding `40px`（内容宽 1180px，对齐 §5.2「工作台最大约 1180px」）、`--es-surface` + 1px `--es-border-default` + `--es-radius-major`、不使用阴影。页面根元素加 `route-surface` 类接入，不复制各自私有的容器样式。容器外 `.app-shell__content` 保留约 16px 浅灰 Canvas 边距。
