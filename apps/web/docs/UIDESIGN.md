...
> 设计方向：Midnight Observatory / Quiet Intelligence  
> 状态：v1.0 原型确认基线  
> 最后更新：2026-08-01  
...
EvidSight 的界面是一台“专业研究仪器”，不是传统后台模板、聊天机器人皮肤或自然风知识产品。视觉需要同时表达：

- 简洁：清楚的区域边界、少量层级和充分留白；
- 高级：精确排版、克制材质与一致细节；
- 神秘感：来自深午夜色、低对比线条、细噪声与空间纵深，而非紫色霓虹；
...
入口页采用全屏叙事流。Hero 使用巨型但受控的中文标题、轨道线和单一亮点。背景的艺术感来自线、噪声和低亮度径向光，不使用照片、自然纹理或动画星空。
...
```css
:root {
  --es-midnight-950: #07090f;
  --es-midnight-900: #090b12;
  --es-obsidian-850: #0d111a;
  --es-obsidian-800: #111522;
  --es-surface-raised: #151a27;

  --es-ink-primary: #f4f5f8;
  --es-ink-secondary: #aeb4c2;
  --es-ink-muted: #717789;
...
1. midnight 到 obsidian 的基础渐变；
2. 右上低透明 moonstone 径向光和左下极弱青色雾；
3. 2%–3% 单色噪声，可附加向边缘消失的 1px 坐标网格。
...
.es-canvas {
  background:
    radial-gradient(circle at 72% 16%, rgb(169 199 216 / 7%), transparent 34%),
    radial-gradient(circle at 12% 86%, rgb(112 211 208 / 3%), transparent 30%),
    linear-gradient(145deg, var(--es-midnight-900), var(--es-midnight-950));
}
```
...
- `Luminous Primary`：高价值创建/进入动作，月白底上带克制白色渐变光；
- `Ghost`：次级操作，透明底和清晰边框；
- `Text`：低风险上下文跳转；
- `Danger`：取消、删除和禁用，仅在明确上下文中使用。

主要按钮高度 40–44px，紧凑按钮 34–36px。按钮必须包含可识别动词，例如“查看报告”“进入现场”“进入知识库”，禁止用低对比小字代替主要操作。

状态：Default、Hover、Focus-visible、Pressed、Loading、Disabled。Loading 保留原宽度并显示动作中的文案，不只显示 Spinner。

### 6.2 输入与搜索

输入框使用暗色填充或透明底加底边，聚焦时显示 moonstone 边框和 2px 外焦点环。Placeholder 只提供格式提示，不代替 Label。
...
选择器展示搜索、按更新时间倒序的知识库列表、owner、可见性、文档量和勾选状态。已选数量出现在触发按钮，不把“480 个可检索文档”当成主要范围概念。

列表行点击与 Checkbox 行为一致；键盘可逐项切换。无结果时保留已选项并提供清除搜索。
...
原型截图是 v1.0 页面结构和视觉方向的基线，不是像素级实现替代品。实现可以因真实数据、响应式和无障碍要求调整，但主要信息层级、壳层、动作位置与视觉语义不得无说明偏离。

完整截图位于 [resource/prototype](../../../resource/prototype)，文件 `01`–`10` 为公共及业务页面，`11`–`20` 为管理中心各分区。
