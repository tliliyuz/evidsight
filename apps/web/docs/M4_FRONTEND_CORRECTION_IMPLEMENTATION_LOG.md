...
- 知识库列表操作列只保留“进入知识库”，编辑和删除迁入详情 Hero；（已被 2026-08-12 §3.9 取代：编辑/删除统一收敛到列表行 `⋮` 菜单，详情 Hero 不再提供知识库级治理动作）
...
### 3.9 列表治理操作收敛到 ⋮ 菜单（2026-08-12）

负责人指示：知识库列表「进入知识库」按钮右侧应像问答历史「打开对话」一样提供 `⋮` 溢出菜单，删除/编辑统一走该菜单；详情 Hero 的「编辑/删除知识库」不再提供。落点：

- 列表行操作列 = 「进入知识库」+ 共享 `RowMenu`（编辑 / 删除知识库），按 `canManageKb`（Owner 或管理员治理）可见，只读成员不显示 `⋮`；
- 编辑复用 `KnowledgeBaseFormDialog`（提交按钮按创建/编辑区分「创建/保存」，修复此前编辑态仍显示「创建」的不一致）；删除具名确认，成功后从列表缓存剔除该行、当前页为最后一项（page>1）时回退页码，全局 Toast「已删除知识库「X」」；
- 详情 Hero 移除「编辑/删除知识库」按钮，保留「用它提问」主按钮与「上传文档」（Owner）；文档 Ledger 的「删除文档」仍按 Owner 或管理员治理权限保留；
- 记录 §3.2「编辑和删除迁入详情 Hero」已被本决定取代，权威事实见 FRONTEND §5.3/§5.4 与 UIDESIGN §7.9/§7.10。

验证（自动化 GREEN，非负责人验收）：vitest 全量 **225 项**全绿（list 新增 Owner/只读/管理员 `⋮` 权限、删除流程、编辑流程 5 项，detail 迁移删除知识库用例为「不展示」断言）；ESLint/Prettier/build 通过；Playwright 知识库 **15 项**重建受影响基线通过（列表含 `⋮` 按钮、管理员治理菜单打开态、详情 Hero 去治理按钮）。
