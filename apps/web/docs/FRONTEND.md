...
创建页收集研究主题、研究类型、来源策略和知识库：

- 研究类型：`comparison` 对比研究 / `explainer` 解释型 / `analysis` 影响分析；
- 来源策略 `knowledge`：必须选择至少一个知识库；`web`：不允许携带内部知识库；`hybrid`：必须选择至少一个知识库，同时启用公开网络；
- 知识库多选（1–50，复用 `kb-picker__*` 浮层，已选名称以 chip 平铺可移除）；只允许选择当前可见知识库；
- `ResearchTaskCreate` 为 `additionalProperties:false`，只发送契约字段（`topic + requirements{task_type,depth,max_sources,language} + source_strategy + knowledge_base_ids`），不发送预算/期望输出等未知字段；不展示「预计 X 分钟」（禁止虚构，deadline 由服务端推导）。

提交使用 `Idempotency-Key`（前端 useRef 缓存，重试复用同 Key 同载荷命中服务端幂等，成功后重置）。成功后立即进入任务运行态，并把研究类型/来源策略/知识库名称通过导航 state 临时传递给运行态信息栏（任务 DTO 不含这些字段，刷新后信息栏降级为只显示 API 字段，见 §5.9）；遇到并发或队列限制（429）时展示限制说明和前往任务列表的入口，不创建本地伪任务。

### 5.8 研究任务列表

研究任务是可管理的任务账本（Ledger）。每行展示题目/类型、来源策略 channel 徽章、状态、最近更新时间和明确操作：运行中“进入现场”，已完成且含报告（`report_id` 非空）“查看报告”（进入 `/reports/:reportId`），其余“查看”。

支持按状态（全部/进行中/已完成/异常，映射后端单状态参数）和关键词筛选；来源策略筛选因后端 `GET /tasks` 暂无 `source_strategy` 参数而未实现（待裁决记录）。删除、取消和恢复遵守 API 权限和幂等语义；删除只允许终态任务（运行中不可删除），具名确认后从缓存剔除、页码回退并给出全局反馈；可恢复失败任务的“继续任务”由运行态页提供（列表 DTO 无 `recoverable`，待裁决记录）。前端不能通过移除列表项假装操作完成。

### 5.9 研究运行态

运行态按 FRONTEND §8 状态机驱动：先 `GET /state` 获取服务端持久快照（LoadingSnapshot → Subscribing），再订阅 Research SSE（Subscribing → Live）；终态以快照为准，`stream.end` 不等于任务成功；断线进入 Reconnecting，自动携带 `Last-Event-ID` 游标重连，关闭页面不取消任务。

页面展示任务标题、连接状态（实时连接/正在重连/连接中）、整体进度（百分比 + 完成/总步骤 + 运行时长）、七阶段 Pipeline（规划/检索/获取/重排/综合/证据图谱/生成，未知非终态阶段降级为“处理中”）、事件时间线和取消入口。事件流只展示可公开的动作摘要、安全计数、耗时、重试和降级，不展示模型隐藏推理（不渲染 `step.updated` 的 `arguments`/`observation`/`output` 原始内容）。
...
取消是“请求”：具名确认后 `POST /cancel`，任务在后台持续运行；可恢复失败任务展示“继续任务”入口，`POST /resume` 成功后重新拉取快照并订阅。刷新或重连后先获取服务端快照，再从事件游标继续投影。

任务信息栏只展示 API 字段（当前证据 `stats.total_evidence`）以及创建表单经导航 state 临时传入的研究类型/来源策略/知识库名称；预算由服务端推导且 DTO 未暴露，不展示（刷新后导航 state 字段降级消失，待裁决记录）。
