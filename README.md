...
**ResearchMind 不是「更好的 AI 搜索」，而是可追溯、可审计的结构化研究引擎。**

在 Perplexity、Gemini Deep Research 这类产品里，用户输入一个主题，等待几十秒，拿到一份答案——但你看不到答案是怎么来的，也无法审计每一个结论的证据是否可靠。ResearchMind 解决的是**研究过程的可信度问题**：每一步 Planning 决策、每一次 Search 调用、每一条 Evidence 提取与排序、最终报告的每一个结论，都有明确的引用锚点和执行状态。

ResearchMind 基于 **Agentic DAG 执行模型** 构建：研究任务被拆解为可并行执行的子步骤，每个步骤拥有独立状态、输入输出契约和证据追踪链。它不是 Chat System、不是 QA System、也不是传统 RAG System，而是一个 **Workflow Engine + LLM System**。
...
| 序号 | 主题 | 描述 |
|:---|:---|:---|
| 1 | [核心特性](#核心特性) | 可审计研究流程、Evidence Graph、断点续跑、Worker 崩溃恢复 |
| 2 | [系统架构](#系统架构) | 核心引擎与表达层分层、7 阶段 Pipeline、三层状态模型 |
| 3 | [技术栈](#技术栈) | 后端、AI、检索、存储、前端、部署各层技术选型 |
...
| 8 | [项目结构](#项目结构) | 后端、Pipeline、前端、测试目录树 |
...
**可审计的 Agentic Research Pipeline**：研究任务按 7 阶段 Pipeline 执行——Planning → Search → Fetch → Rerank → Synthesis → Evidence Graph Build → Report Render。每个阶段独立可观测，Step 级状态、耗时、Token 消耗全部持久化。
...
**SSE 实时研究过程**：15 种 SSE 事件类型（task.* / phase.* / step.* / checkpoint.*）实时推送，客户端断线后重连可立即获得完整状态快照，并基于 `seq` 序号丢弃乱序事件。
...
ResearchMind 架构分为两层：**核心引擎产出 Evidence Graph，表达层将其渲染为不同形态的报告**。
...
架构详情见 [ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
| LLM | deepseek-v4-pro (DeepSeek SDK) | MVP 单一模型，后续按阶段分级 |
| 搜索 | Tavily API | 含内容提取 |
...
Pipeline 七阶段的输入/输出数据结构、Prompt 模板、失败策略、SSE 事件映射详见 [RESEARCH_PIPELINE.md](docs/RESEARCH_PIPELINE.md)。
...
Pipeline 七阶段进度条、Step 实时日志、已耗时与取消操作。
...
│   ├── models/                  # SQLAlchemy ORM 模型（User / ResearchTask / ResearchStep / ResearchSource / EvidenceItem / ReportSection / RefreshToken + UTCDateTime）
...
│   │   └── sse_bridge.py        # Pipeline 事件 → SSE 事件桥接
...
| [ARCHITECTURE.md](docs/ARCHITECTURE.md) | 架构设计文档 — 技术选型、7 阶段 Pipeline、三层状态模型、权限模型 |
...
**Q: 断点续跑是什么意思？**
每个 Step 完成后，系统会把当前阶段、已完成 Step、执行指针、进度等写入 `execution_context`。如果 Worker 崩溃或任务失败，用户可以点击「断点续跑」，系统会从最后一个完成 Step 的下一个 Step 继续执行，而不是从头开始。
