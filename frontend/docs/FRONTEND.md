...
> - `ResearchPage.vue` 在 `onMounted` 中检测 `current.status === 'running' && sseStatus === 'disconnected'` 时自动调用 `connectSSE(task_id)`，恢复实时流并继续追加日志。
>
...
> **实现模块**：`utils/markdown.js` — markdown-it + highlight.js（github-dark 主题）+ 自定义 `[来源N]` 引用锚点 plugin（正则匹配 → `<a class="citation-link" data-evidence-index="N">`）。[Deviation] 多索引时 `data-evidence-index` 使用空格分隔（如 `"0 1"`），支持精确 CSS 选择器 `[data-evidence-index~="N"]`；原逗号分隔方案在实际联动中发现无法精确匹配单条 Evidence，故调整。来源：DocMind `frontend/src/utils/markdown.js`，ResearchMind 扩展引用锚点解析。
