...
| 6 | `building_evidence_graph` | 来源图谱 | `fa-sitemap` |
...
右侧固定面板的上半部分。当来源条目较多时，列表在面板固定高度区域内独立滚动，不挤占下方的 Trace 摘要。

```css
.evidence-panel {
    width: 100%;                                     /* 由父级 grid 列宽 / --rm-evidence-panel-width 控制 */
    background: var(--rm-bg-page);
    border-left: 1px solid var(--rm-border);
    overflow-y: auto;                                /* 独立使用时整体滚动 */
    padding: var(--rm-space-3);
    display: flex;
    flex-direction: column;
    min-height: 0;                                   /* 在 flex 容器内允许被压缩 */
}

.evidence-list {
    flex: 1;
    overflow-y: auto;                                /* 在侧栏内部分区滚动 */
    min-height: 0;
}
```
...
位于右侧固定面板下半部分，固定在侧栏底部，默认折叠，不受来源列表滚动影响。
...
.trace-panel {
    ...
    margin-top: var(--rm-space-3);                 /* 与上方来源面板分隔 */
}
...
| `--rm-transition-slow` | 0.3s ease | Pipeline 阶段切换、来源联动闪烁消退 |
