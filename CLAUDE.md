...
任何 Markdown 新增、修改、删除或移动前，必须完整阅读 [文档格式规范](docs/guides/DOCUMENT_FORMAT.md) 与 [文档治理指南](docs/guides/DOCUMENT_GOVERNANCE.md)，再确定文档类型、唯一权威来源和是否需要 ADR 检查；一行修订、链接修复、纯文档变更和批量重构同样适用。章节编号、状态词、模板、链接和冗余边界以这两份指南为准；正文只保留权威事实，投影使用链接引用。

- 新增和修改内容必须遵循对应文档模板、连续编号、条目语法、状态分层和冗余边界，不得把实施清单塞入职责不匹配的文档；
- 章节改号、文件移动或事实变更必须同步全部引用、目录入口和必要投影；同一完整清单只能保留一个权威版本。

文档变更完成后必须执行 `bash scripts/check-document-format.sh`、`bash scripts/check-adr-governance.sh`（涉及 ADR 或根级治理入口时）和 `git diff --check`。格式、链接、镜像或索引检查失败时，不得进入评审或提交。
...
根目录 `AGENTS.md` 与 `CLAUDE.md` 是逐字节一致的治理入口镜像，不是两个独立事实源。

- 修改任一文件时，必须在同一变更中同步另一文件；
- 禁止只提交其中一个文件的修改，并必须通过 `cmp -s AGENTS.md CLAUDE.md`；
- 产品、架构、接口、数据和界面事实不得写入根级入口，必须以权威文档矩阵指定的来源为准；
- 两文件不一致时，文档治理和相关实现均视为阻断。
...
| 文档格式、模板、编号与冗余边界 | `docs/guides/DOCUMENT_FORMAT.md` |
...
- 新建、移动或拆分代码文件前，先按 `docs/guides/DEVELOPMENT.md` 的目录与模块边界核验；新增顶层目录、跨服务边界或新公共契约时，必须先补齐规范并重新执行 ADR 检查。
...
- REFACTOR 结束后必须将实际发生的结构性变更写入 `docs/CHANGELOG.md` 的带日期一级条目；未发生的计划不得写入 Changelog。
