...
- Admin 侧边栏包含：系统统计 + 知识库管理 + 文档管理 + 返回对话
- 两者完全独立，不混用
...
7 张统计卡片：用户总数、知识库数、文档总数、总会话数、分块总数、消息总数、存储空间。含快捷管理入口卡片。ECharts 可视化待后续排期。
...
| 变更角色 | 确认弹窗（`ElMessageBox.confirm`）→ `PUT /api/admin/users/{user_id}/role` |
...
| Admin Stats (`/admin/stats`) | ✅ 已实现 | 7 项统计卡片 + 千分位/存储格式化 + 快捷管理入口 | ECharts 可视化（Phase 5） |
...
| ECharts 图表 | ⬜ 待实现 | 问答量趋势/响应时间/Token 使用三个图表 | Phase 5 |
