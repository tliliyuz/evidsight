...
本目录集中项目级、具有约束力的规范。规范回答“系统必须是什么、允许什么、如何验收”；开发操作见 [`guides/`](../guides/DEVELOPMENT.md)，阶段安排与迁移步骤见 [`plans/`](../plans/ROADMAP.md)，重要选择的历史原因见 [`decisions/`](../decisions/README.md)。
...
| 运行与恢复 | [`OPERATIONS.md`](OPERATIONS.md) | 部署、观测、备份、恢复和故障处理 |

## 阅读顺序

1. 先读 PRD 确认范围和验收目标。
2. 再读总体架构和身份规范确认服务与安全边界。
3. 按任务读取 API、配置、测试、数据迁移、保留或运维规范。
