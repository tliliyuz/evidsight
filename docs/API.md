...
> **实现状态**：以下管理后台接口均排期至 **Phase 5** 实现。当前 Phase 2 后端 `admin.py` 为空，router 未注册。前端 `/admin/*` 页面依赖这些接口，Phase 2.3.3 仅完成前端页面开发，接口联调待 Phase 5。

| 接口 | 说明 | 实现 |
|:---|:---|:---|
| `GET /api/admin/stats` | 系统概览统计 | Phase 5 |
| `GET /api/admin/documents` | 全部文档列表（跨库） | Phase 5 |
| `GET /api/admin/knowledge-bases` | 全部知识库列表（跨用户） | Phase 5 |
...
| POST | `/api/auth/login` | 公开 | 登录 | Phase 1 ✅ |
| POST | `/api/knowledge-bases` | user | 创建知识库 | Phase 2 ✅ |
| GET | `/api/knowledge-bases` | user | 知识库列表（仅当前用户） | Phase 2 ✅ |
| GET | `/api/knowledge-bases/{id}` | user（所有者/admin） | 知识库详情 | Phase 2 ✅ |
| PUT | `/api/knowledge-bases/{id}` | user（所有者/admin） | 更新知识库 | Phase 2 ✅ |
| DELETE | `/api/knowledge-bases/{id}` | user（所有者/admin） | 删除知识库（异步） | Phase 2 ✅ |
| POST | `/api/knowledge-bases/{kb_id}/documents` | user（所有者/admin） | 上传文档（支持 force 覆盖） | Phase 2 ✅ |
| POST | `/api/knowledge-bases/{kb_id}/documents/batch-upload` | user（所有者/admin） | 批量上传文档 | Phase 2 ✅ |
| POST | `/api/knowledge-bases/{kb_id}/documents/{id}/reprocess` | user（所有者/admin） | 重新处理失败文档 | Phase 2 ✅ |
| GET | `/api/knowledge-bases/{kb_id}/documents` | user（所有者/admin） | 文档列表（支持筛选/排序） | Phase 2 ✅ |
| GET | `/api/knowledge-bases/{kb_id}/documents/{id}` | user（所有者/admin） | 文档详情 | Phase 2 ✅ |
| GET | `/api/knowledge-bases/{kb_id}/documents/{id}/chunks` | user（所有者/admin） | 查看分块（分页，生产截断） | Phase 2 ✅ |
| DELETE | `/api/knowledge-bases/{kb_id}/documents/{id}` | user（所有者/admin） | 删除文档（异步） | Phase 2 ✅ |
...
| POST | `/api/chat` | user | 问答（SSE） | Phase 3 |
