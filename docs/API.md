...
| 字段 | 类型 | 必填 | 说明 |
|:---|:---|:---|:---|
| files | file[] | 是 | 多文件（每个 ≤ 50MB，单次最多 50 个） |
...
```
event: sources
data: {"chunks": [{"chunk_index": 1, "doc_id": 5, "doc_name": "报销制度.md", "content": "差旅报销需提交：1. 差旅申请单...", "score": 0.92, "page": 3, "section_title": "§3 差旅报销流程", "section_path": "报销制度 > §3 差旅报销流程", "preview_text": "差旅报销需提交：1. 差旅申请单 2. 交通票据...", "preview_range": {"start": 0, "end": 200}, "highlight_start": 0, "highlight_end": 18}]}
```
...
| chunks[].section_title | string \| null | 当前所属章节标题（如 `§6.1 SSE 事件完整格式`），来自 Markdown 标题解析 |
| chunks[].section_path | string \| null | 章节路径（如 `RAG Pipeline > §6 SSE 事件流`），层级用 ` > ` 分隔 |
