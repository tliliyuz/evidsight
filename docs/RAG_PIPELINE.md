...
[证据审查] Evidence Review → chunk 角色分类 + 门控决策                 ← [Phase 5.5 ADR-021]
	  ...
	  └─ REJECT → 跳过 LLM，直接返回「未找到相关信息」
	    ↓
[Prompt] 组装 Prompt → 宽松 Prompt 策略 + 检索结果            ← [Phase 5.5 回退为宽松策略]
	    ↓
...
- `store.search(query_embeddings=[vec], n_results=10, kb_id=kb_id)` — 通过 `kb_id` 路由到对应 `kb_{kb_id}` collection，无需 `where` 过滤
- `where` 参数保留用于可选的 doc 级过滤（如 `where={"doc_id": 42}`），不再用于 KB 级隔离
- metadata 值为数值类型（int），入库和查询两端统一使用 int，无需类型转换

> **Collection 策略（2026-06-18 更新）**：已从「单 collection + metadata 隔离」迁移为 **Per-KB Collection**（`kb_{kb_id}`）。根因：带 `where` metadata 过滤的查询耗时 16.4s vs 无过滤 35ms，ChromaDB 的 metadata filter 不会利用 HNSW 索引加速。迁移后向量查询直接命中对应 KB collection，消除 `where` 过滤开销。
...
#### BM25_MAX_CHUNKS 硬限制（超大 KB 保护）

> 配置项：`BM25_MAX_CHUNKS`（默认 10000）。chunk 数超过此阈值的知识库 **完全跳过 BM25 检索**，仅依赖向量检索。根因：21k chunks KB 的全量加载 + jieba 分词内存峰值超过 512MB 容器限制（OOM Kill exit 137）。

**三层检查点**：

| 检查点 | 位置 | 行为 |
|:---|:---|:---|
| ① COUNT 先于 SELECT | `_load_and_cache()` | MySQL `COUNT(*)` 查询先于全量 `SELECT`，超限则 Redis 缓存 `{"skipped": true, "chunk_count": N}`（短 TTL 60s）并返回空 |
| ② Redis `chunk_count` 校验 | `_get_bm25_index()` Redis 命中时 | 先检查 `skipped` 标志（直接跳过），再检查 `chunk_count`（向后兼容：旧缓存无此字段时回退到 `len(tokens)`），超限则跳过 BM25 构建 |
| ③ 进程内缓存 `len(doc_ids)` 校验 | `_get_bm25_index()` L1 命中时 | 超限则清除缓存条目并降级到 Redis/MySQL 路径（由检查点 ② 拦截） |

**权衡**：超大 KB 失去 BM25 关键词召回路，但「能回答」优先于「回答好」；未来可增大容器内存或迁移 BM25 到 Elasticsearch 后恢复。
...
请用中文回答，引用来源时标注 [来源N]（N 为文档编号），并尽量附带文档名和章节信息。"""
...
> **章节信息引用（2026-06-18）**：引用指令从「标注 [来源N]」升级为「标注 [来源N]，并尽量附带文档名和章节信息」，配合 `_format_chunk_reference()` 输出的 `文档: X | 章节: Y | 页码: Z` 格式，引导 LLM 在回答中附带章节信息。
