...
**问题**：Celery Worker 被 SIGKILL/OOM/断电杀死后，任务永久卡在 `running`（死锁）。`task_time_limit=900s` 超时强杀同效。
...
| 并发层 | 任务级幂等锁 | 防止同一任务被两个 Worker 同时恢复 |

**关键设计决策**：

| 决策 | 值 | 理由 |
|:---|:---|:---|
| 任务锁 TTL | 900s（= `task_time_limit`） | 正常任务不会超时；崩溃后旧锁到期自动释放，无需手动清理 |
| 启动恢复阈值 | 1800s（2× `task_time_limit`） | 充分安全边际，避免误判正在执行的任务为过时 |
...
> Pipeline 入口三元检查、`_start_task()` 恢复路径、`_create_step()` 复用逻辑等实现细节见 [RESEARCH_PIPELINE.md §10.5](RESEARCH_PIPELINE.md#105-worker-崩溃恢复)。启动恢复实现见 `app/main.py` `lifespan()`。任务锁实现见 `app/tasks/lock.py`。
...
| **Worker 崩溃恢复防并发** | 任务级锁 `SET rm:task_lock:{task_id} NX EX 900`（TTL = `task_time_limit`），`run()` 入口获取、`finally` 释放 | 两个 Worker 同时恢复同一任务 |
...
- **Worker 崩溃恢复**：`acks_late=True` 保证未完成任务自动重投递；Pipeline 入口识别 `running` 状态进入恢复路径；任务级锁防并发恢复；启动时自动检测过时 `running` 任务并 re-queue（见 §5.4.1）。
