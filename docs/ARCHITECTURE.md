...
- 在 2C2G 单机试点环境中可部署、可诊断、可备份、可恢复和可回滚；
- 未来在不改变业务契约的前提下拆分基础设施或迁移到多节点平台。
...
7. **渐进演进**：首版采用单机 Compose 和共享基础设施实例，但数据库、账号、队列、Key 与卷保持可拆分边界。
...
外部用户只访问 Nginx。Knowledge Service 和 Research Service 不直接暴露到公网；`/internal/v1/*` 只允许 Docker 私有网络中的授权服务访问。
...
Research Beat 必须保持单实例，避免重复调度。两个 Celery Worker 不得注册或消费对方任务。
...
- 宿主机只对用户网络开放 Nginx 的 `80/443`。
- MySQL、Redis、Knowledge API、Research API 和 Internal API 不映射公网端口。
- 管理或诊断端口如需临时开放，只能绑定 `127.0.0.1` 并在操作完成后关闭。
- Nginx 禁止将 `/internal/v1/*` 配置为外部路由。
- 两个 API 可位于 edge 与 internal 网络；数据组件和 Worker 只加入 internal 网络。
- 容器通过服务名发现，不在应用中硬编码容器 IP。
...
Knowledge API 与 Knowledge Worker 仅共享 Knowledge-owned 文件和向量卷。Research Service 禁止挂载这些卷。
...
- 2 vCPU；
- 2 GB RAM；
- 持久磁盘容量由试点文档量确定，部署前至少预留 20 GB 可用空间；
- Docker Engine 与 Docker Compose v2。

该环境面向 10–30 名试点用户和低并发，不承诺所有用户同时执行重任务。

### 11.2 默认资源策略

| 组件 | 建议内存上限 | 策略 |
|:---|---:|:---|
| MySQL | 448 MiB | 限制 buffer pool 和连接数 |
| Redis | 96 MiB | 设置 `maxmemory` 与 `noeviction`；缓存写失败时回源，Broker 数据不可静默淘汰 |
...
3. 互斥调度 Knowledge 入库与 Research 重任务；
...
- 2C2G 默认启用跨服务重任务准入锁：文档入库与深度研究不可同时占用重任务执行槽；未取得执行槽的任务保留在原队列等待，不转移到对方队列。
...
2C2G 基线不默认常驻 Prometheus/Grafana。试点可由运维按需抓取指标，或在另一主机部署监控组件。
...
1. MySQL 三个逻辑数据库的一致性备份；
2. Knowledge uploads 与 Chroma 卷的同一备份批次快照；
...
2. 运行向后兼容的数据库迁移；
3. 更新 Worker 和 Beat；
4. 更新 Knowledge API 与 Research API；
5. 更新 Web 静态资源与 Nginx；
...
- 在配置内存限制的 2C2G 环境启动全部核心组件，不默认启动 Prometheus/Grafana。
- 同时提交一个文档入库任务和一个深度研究任务时，系统通过排队或互斥策略避免 OOM，任务不丢失。
- 达到并发或队列上限时返回明确可重试错误。
...
达到任一条件时，评估从单机基线演进：持续内存压力、队列等待不可接受、单机故障风险超过业务容忍度、备份恢复无法满足目标或试点扩大为部门级生产。

推荐演进顺序：

1. 主机升级至至少 4C8G，并启用独立监控；
2. 将 MySQL 和备份迁至托管或独立数据主机；
3. 将 Redis 按 Knowledge/Research 或 cache/broker 物理拆分；
