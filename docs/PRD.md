...
> 本文档为 ResearchMind 的产品需求真理源，仅描述「做什么 / 为什么 / 给谁做 / 做到什么范围」。架构决策见 [ARCHITECTURE.md](ARCHITECTURE.md)，管线详设见 [RESEARCH_PIPELINE.md](RESEARCH_PIPELINE.md)，接口详设见 [API.md](API.md)，表结构细节见 [DATABASE.md](DATABASE.md)，版本排期见 [ROADMAP.md](ROADMAP.md)——上述文档通过交叉引用链接到本文档对应章节。
...
ResearchMind 是**完全独立的项目**，不与 DocMind 共享数据库、代码或运行时。本节仅作产品定位对比——两者解的是同类「知识获取」问题的不同侧面，ResearchMind 在工程结构（分层、表设计、Auth、错误码体例）上借鉴了 DocMind 的设计思路，但已全部内化为自有实现。
...
> 三种任务类型映射到不同的 Planner 策略、Reranker 排序维度、Report 模板。各类型的 Pipeline 策略映射见 [ARCHITECTURE.md §2.2](ARCHITECTURE.md#22-pipeline-七阶段定义) 和 [RESEARCH_PIPELINE.md §2.4](RESEARCH_PIPELINE.md#24-planner-策略按-task_type)。
