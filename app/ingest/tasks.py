...
async def _ingest_document_async(doc_id: int) -> dict:
    ...
    try:
                ...
                if not chunk_rows:
                    ...
                    if resume_stage == "vector_storing":
                        ...
                        try:
                            ...
                            logger.exception("文档 %d ChromaDB 清理失败，继续执行", doc_id)
        ...
        logger.exception("文档 %d 入库流水线异常: %s", doc_id, e)
        try:
            async with async_session() as db:
                doc = await db.get(Document, doc_id)
                if doc is not None:
            ...
            logger.exception("文档 %d 异常状态更新也失败了", doc_id)
...
async def _delete_document_async(doc_id: int) -> dict:
    ...
    try:
        ...
        # 5. 物理删除 MySQL 记录（FK CASCADE 自动清理 chunks）
        async with async_session() as db:
            doc = await db.get(Document, doc_id)
            if doc is not None:
                await db.delete(doc)
                await db.commit()
                logger.info("文档 %d MySQL 记录已物理删除", doc_id)

        return {"status": "completed", "doc_id": doc_id}

    except Exception as e:
        logger.exception("文档 %d 异步删除异常: %s", doc_id, e)
        return {"status": "error", "doc_id": doc_id, "error": str(e)}
...
def delete_document(self, doc_id: int) -> dict:
    ...
    loop = asyncio.new_event_loop()
    try:
        return loop.run_until_complete(_delete_document_async(doc_id))
    finally:
        loop.close()
