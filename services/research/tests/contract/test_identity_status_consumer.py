...
class TestIdentityStatusConsumer:
    ...
    def test_consumer_can_parse_valid_identity_status(self):
        ...
        for name in self.VALID:
            errors = list(
                _validator().iter_errors(load_fixture("identity-status-response", "valid", name))
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            f
            for f in list_fixtures("identity-status-response", "invalid")
    ...
    def test_consumer_rejects_missing_required_fields(self, fixture_name):
        ...
        errors = list(
            _validator().iter_errors(
                load_fixture("identity-status-response", "invalid", fixture_name)
            )
        )
