...
class TestResearchV1Create:
    ...
    async def test_缺少IdempotencyKey_返回422(self, async_client: AsyncClient, auth_headers: dict):
        ...
        assert r.json()["error"]["error_code"] == "SYSTEM_VALIDATION_FAILED"
    ...
    async def test_同Key同载荷_重放同一task且replayed_true(
        ...
        assert r1.json()["idempotent_replayed"] is False
        assert r2.json()["idempotent_replayed"] is True
    ...
    async def test_同Key不同载荷_返回409E2009(self, async_client: AsyncClient, auth_headers: dict):
        ...
        assert r2.json()["error"]["error_code"] == "RS_IDEMPOTENCY_CONFLICT"
    ...
    async def test_客户端预算摘要被拒绝(self, async_client: AsyncClient, auth_headers: dict):
        payload = _payload()
        payload["budget_summary"] = {"max_tokens": 1000}
        r = await async_client.post(
            "/api/v1/research/tasks", json=payload, headers=_headers(auth_headers)
        )
        assert r.status_code == 422
        assert r.json()["error"]["error_code"] == "SYSTEM_VALIDATION_FAILED"
