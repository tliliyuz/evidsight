...
class TestExecuteKnowledge:
    ...
    async def test_正常检索流程(self):
        ...
        with (
            patch("app.rag.knowledge_pipeline.needs_rewrite", return_value=False),
    ...
    async def test_检索失败抛RetrievalServiceException(self):
        ...
        mock_vector.search = AsyncMock(side_effect=Exception("向量存储连接失败"))
    ...
    async def test_触发查询重写(self):
        ...
        with (
            patch("app.rag.knowledge_pipeline.needs_rewrite", return_value=True),
            patch(
                "app.rag.knowledge_pipeline.rewrite_query",
