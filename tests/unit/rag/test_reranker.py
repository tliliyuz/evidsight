"""Rerank 重排序模块单元测试 — 覆盖 NoopReranker + DashScopeReranker 核心逻辑

对齐 ROADMAP.md §5.5 Phase 3 测试 + §8.6 Phase 5.5 测试：
- NoopReranker: 保持 RRF 原始排序（不再按长度重排）、截取 top_k、边界处理
- DashScopeReranker: API 正常响应排序、异常降级回退、空输入/单输入边界、接口一致性
"""
