...
// ===== Token 自动刷新机制 =====
// 对齐 FRONTEND.md §1.3.1：401+E1003 → refresh → 重放原请求
...
api.interceptors.response.use(
  ...
  async error => {
    ...
    // E1002：用户名或密码错误 / E1010：用户被禁用 → 非 token 问题，透传错误给调用方
    if (code === 'E1002' || code === 'E1010') {
      return Promise.reject(error)
    }
    ...
    // 其他 401（E1004/E1005/E1006/E1007/E1008/E1009 等）→ 清除 token 跳转登录
    clearAndRedirect()
    return Promise.reject(error)
  }
)
