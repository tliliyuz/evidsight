"""句级 Evidence 定位 + 修辞角色过滤 — 复用 BM25 在段落内部定位最佳证据句

对齐 RESEARCH_PIPELINE.md §5.3（BM25 粗筛）：
- 对每个段落切句 → 修辞角色过滤 → BM25Okapi → 取 argmax
- 不新增算法：复用 rank-bm25 + jieba（与 bm25.py 一致）
- 确定性：同一 question 永远返回同一句子
- 每个段落独立构建微型 BM25 索引（3-8 句），
  IDF 天然区分关键词权重差异
...
from app.pipeline.types import SearchOutput
...
def match_sentences(output: SearchOutput, question: str) -> SearchOutput:
    """对每个检索结果内部做句级 BM25 定位，记录最佳证据句。
    ...
    Returns:
        同一 SearchOutput，每个 result 的 matched_sentence 已填充
    """
